namespace Compiler.Module
{
    internal static class GrammarConst
    {
        internal const string NumberId = "number";
        internal const string StringId = "string";
        internal const string IdentifierId = "identifier";
        internal const string BaseCallId = "baseCall";
        internal const string ParametersId = "parameters";
        internal const string SingleParameterId = "parameter";
        internal const string FunctionId = "function";
        internal const string LineId = "line";
        internal const string LinesId = "lines";
        internal const string CalltId = "call";
        internal const string SimpleCallId = "simpleCall";
        internal const string FunctionCallId = "functionCall";
        internal const string FunctionThreadCallId = "threadFunctionCall";
        internal const string MethodCallId = "methodCall";
        internal const string MethodThreadCallId = "methodThreadCall";
        internal const string Thread = "thread";
        internal const string BlockId = "block";
        internal const string RootStatementId = "rootStatement";
        internal const string RootStatementsId = "rootStatementsId";
        internal const string ParsParametersId = "parsParameters";
        internal const string LeftParId = "leftPar";
        internal const string RightParId = "rightPar";
        internal const string CommaId = "comma";
        internal const string LeftBraceId = "leftBrace";
        internal const string RightBraceId = "rightBrace";
        internal const string SemicolonId = "semicolon";
    }
}