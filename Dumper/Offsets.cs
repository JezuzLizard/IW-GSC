...
namespace Dumper
{
    public static class Offsets
    {
        ...
        public static void SetupOffsetsForGameId(Game.GameId gameId)
        {
            switch (gameId)
            {
                case Game.GameId.Ghost:
                    AssetsPool = FindPattern(0x140001000, 0x145000000, "\x4C\x8D\x05\x00\x00\x00\x00\xF7\xE3", "xxx????xx");
                    uint offset = BitConverter.ToUInt32(Memory.Read(AssetsPool + 3, 4), 0);
                    AssetsPool += offset + 7;
                    break;
                default:
